...
<script type="text/javascript">
//발급
function CertProcessIssue(caName, szRef, szCode) {
	var frm = document.readForm;
	var isHtml5 = frm.isHtml5.checked;
		
	INIWEBEX.reissueCertificate({
		processCallback	: "ReissueCertificateCallback",	// 콜백함수
...
function ReissueCertificateCallback(result){
	if(result){
...
<div class="ini-demo-wrapper">
	...
	<main class="ini-demo-container">
		...
		<section class="ini-demo-contents-wrapper" id="content">
			<form name="readForm" id="readForm" method="POST" action="./reissue_complete.jsp" onSubmit="return false;">
			...
			%>
				...
				<div class="dl-table">
					<dl>
						...
						<dd class="col-sm-9 col-xs-9"><input type="checkbox" name="isHtml5"  /> (※ 체크 시 HTML5 UI 사용)</dd>
					</dl>
				</div>
				<div class="ini-demo-btns">
					<button type="button" class="btn btn-info btn-lg" onClick="CertProcessIssue('<%=caName%>', '<%=oppMsg.getCodeData("REFNUM")%>','<%=oppMsg.getCodeData("APPCODE")%>')" >공인인증서 신규발급</button>
