...
%><%
	...
	if( ACTION_REQ_AUTHCODE.equals(action) ) {
		...
		if( authCodeSize < MIN_AUTHCODE_SIZE || authCodeSize > MAX_AUTHCODE_SIZE ) {
			out.print("ERROR$INVALID_AUTHCODE_SIZE$EXCEPTION CODE LENGTH (LENGHT:"+MIN_AUTHCODE_SIZE+"~"+MAX_AUTHCODE_SIZE+")");
			return;
		}
	...
	/** ActiveX 에서 전송된 인증서 저장 처리 부분 */
		...
		if( strRegNum == null || strEncCert == null ) {
			out.print("ERROR$NO_PARAMS$UNREGISTED CODE OR CETIFICATION.");
			return;
		}
		
		Object storedObject[] = (Object[])certTable.get( strRegNum );
		if( storedObject == null ) {
			out.print("ERROR$INVALID_AUTHCODE$INVALID AUTHENTICATION CODE.");
			return;
		}
