...
%><%
	...
	*/
	...
	if( ACTION_EXPORT.equals(action) ) {
		if( strEncCert == null ) {
			out.print("ERROR$NO_PARAMS$REQUESTED CERTIFICATION IS NULL.$");
			return;
		}
		...
		try {
			...
			out.print("ERROR$INVALID_SIZE$MISS MATCHED CODE LENGTH.$");
			return;
		}
		
		if( authCodeSize % AUTHCODE_BLOCK_SIZE != 0 ) {
			out.print("ERROR$INVALID_AUTHCODE_SIZE$MISS MATCHED CODE BLOCK SIZE.$");
			return;
		}
		if( authCodeSize < MIN_AUTHCODE_SIZE || authCodeSize > MAX_AUTHCODE_SIZE ) {
			out.print("ERROR$INVALID_AUTHCODE_SIZE$EXCEPTION CODE LENGTH (LENGTH:"+MIN_AUTHCODE_SIZE+"~"+MAX_AUTHCODE_SIZE+").$");
			return;
		}
		...
		if( generatedCode != null ) {
			...
			out.print("ERROR$TOO_MANY_SERVICES$SERVER BUZY.$");
		}
	}
		...
		if( strRegNum != null ) {
			...
			if( target != null ) {
				...
				if( !STATUS_RECEIVE.equals(statusV) && !STATUS_SEND.equals(statusV) ) {
					out.println("ERROR$NO_CERT$UNKOWN STATUS("+statusV+").$");
					return;
				}
				if( target[1] != null ) {
					...
					*/
					...
					out.println("ERROR$NO_CERT$UNREGISTED CETIFICATION.$");
					return;
				}
			} else {
				out.println("ERROR$BAD_PARAMS$MISS MATCHED CODE OR EXPIRED TIMEOUT.$");
				return;
			}
		} else {
			out.println("ERROR$NO_PARAMS$REQUESTED CODE IS NULL.$");
			return;
		}
	}
	/** 인증서 목록 조회 (브라우져) */
		...
		if(strRegNum != null){		
			...
			if( target != null ) {
				if( target[2] != null ) {
					...
					out.println("ERROR$NO_STATUS$UNREGISTED CETIFICATION.$");
					return;
				}
			}else{
				out.println("ERROR$NO_STATUS$MISS MATCHED CODE OR EXPIRED TIMEOUT.$");
				return;
			}
			...
			if(ht_status != null){
					...
					if(target != null){
							...
							if(ht_status.equals( STATUS_COMPLETE )){					
								...
								out.println("ERROR$INVALID_PARAMS$UNKOWN STATUS[" + ht_status + "].$");
								return;
							}
					}else{
							out.println("ERROR$NO_PARAMS$MISS MATCHED CODE OR EXPIRED TIMEOUT.$");
							return;
					}
				}else{
					out.println("ERROR$NO_PARAMS$REQUESTED STATUS IS NULL.$");
					return;
			}
		}else{
			out.println("ERROR$NO_PARAMS$REQUESTED CODE IS NULL.$");
