...
var TOUCHENEX_DAEMON = {
	...
	moduleCheck : function( currPluginCnt ){
		...
		if(currPluginCnt >= TOUCHENEX_CONST.pluginCount){
			...
			exlog("_DAEMON.moduleCheck.chkInfoStatus", this.chkInfoStatus);
		...
		try {
			if(sessionStorage.getItem('exws')){
				exlog("_DAEMON.moduleCheck.portCheck.exws", sessionStorage.getItem("exws"));
				TOUCHENEX_UTIL.sendWS(pluginInfo.exEdgeInfo.localhost, request, checkCallback);
			} else {
				if(typeof(Worker) !== "undefined") {
					var exwsWorker;
					if(typeof(exwsWorker) == "undefined") {
						exwsWorker = new Worker(pluginInfo.exEdgeInfo.portChecker);
					}
					exwsWorker.onmessage = function(event) {
						exlog("_DAEMON.moduleCheck.portCheck.result",event.data);
						if(event.data != "false"){
							sessionStorage.setItem('exws', event.data);
							exlog("_DAEMON.moduleCheck.exws", sessionStorage.getItem("exws"));
							TOUCHENEX_UTIL.sendWS(pluginInfo.exEdgeInfo.localhost, request, checkCallback);
						} else {
							exlog("_DAEMON.moduleCheck.portCheck", "cwex daemon port not found");
							TOUCHENEX_CHECK.setDaemonStatus("", false, true);
						}
						exwsWorker.terminate();
					};
					exwsWorker.onerror = function(e){
						exlog("_DAEMON.moduleCheck.portCheck [exception]", e);
						TOUCHENEX_CHECK.setDaemonStatus("", false, true);
						exwsWorker.terminate();
					};
					var exwsParam = {};
					exwsParam.debug = TOUCHENEX_CONST.debug;
					exwsParam.port = pluginInfo.exEdgeInfo.edgeStartPort;
					exwsParam.localhost = pluginInfo.exEdgeInfo.localhost;
					exwsParam.chkCnt = pluginInfo.exEdgeInfo.portChkCnt;
					exwsWorker.postMessage(exwsParam);
				} else {
		...
		try {
			...
			if ("WebSocket" in window){
				...
				if(touchenexWS.readyState == 1){
					...
					var url = host + ":" + sessionStorage.getItem("exws") + "/" + touchenexInfo.exSiteName + "/" + touchenexInfo.exProtocolName + "/Call";
...
var TOUCHENEX_EX = function( property ) {
	...
	this.InvokeDaemon = function( id, cmd, fname, args, callback ) {
		...
		try {
			...
			TOUCHENEX_UTIL.sendWS(this.host, request, "TOUCHENEX.InvokeCallback");
		} catch (e){
