...
p7.messageFromPem = function(pem) {
  ...
  if(obj.value.length === 2) {
      ...
      // 금결원 포멧
	  return p7.messageFromAsn1Ex(obj);
  }
};
...
p7.messageToPem = function(msg, maxline) {
	...
	if('isRemoveContentInfo' in msg && msg.isRemoveContentInfo === true) {
		if(msg.contentInfo.value.length === 2) {
			if(msg.contentInfo.value[1].type === asn1.Type.NONE) {
				msg.contentInfo.value.pop();
			}
		}
	}
	
	if('isRemoveCertificate' in msg && msg.isRemoveCertificate === true) {
		msg.certificates.pop();
	}
	
	if('isRemoveContext' in msg && msg.isRemoveContext === true) {
	  // context 없는 금결원 포멧
	  pemObj = {
	    type: 'PKCS7',
	    body: asn1.toDer(msg.toAsn1WithoutContext()).getBytes()
	  };
	} else {
...
p7.messageToPemWithoutContext = function(msg, maxline) {
  // convert to ASN.1, then DER, then PEM-encode
  var pemObj = {
    type: 'PKCS7',
    body: asn1.toDer(msg.toAsn1WithoutContext()).getBytes()
...
p7.messageFromAsn1Ex = function(obj) {
  var msg = p7.createSignedData(); 
  msg.fromAsn1(obj);
...
p7.createSignedData = function() {
  ...
  msg = {
    ...
    addSigner: function(signer) {
      ...
      // ensure OID known for signature algorithm
      var signatureAlgorithm = signer.signatureAlgorithm || forge.pki.oids.rsaEncryption;
      switch(signatureAlgorithm) {
      case forge.pki.oids.rsaEncryption:
      case forge.pki.oids['RSASSA-PSS']:
        break;
      default:
        throw new Error(
          'Could not add PKCS#7 signer; unknown message signature algorithm: ' +
          digestAlgorithm);
      }
      ...
      var unauthenticatedAttributes = signer.unauthenticatedAttributes || [];
        ...
        for(var ai = 0; ai < signer.authenticatedAttributes.length; ++ai) {
          ...
          if(attr.type === forge.pki.oids.messageDigest){
        	  if(attr.value && attr.value == 'FALSE'){
        	      ...
        	      content = content.value[0];
        	      // serialize content
        	      content.value = new forge.util.ByteStringBuffer(forge.util.binary.base64.decode(decodeURIComponent(content.value))).data;
        	      var bytes = asn1.toDer(content);
        	      // skip identifier and length per RFC 2315 9.3
        	      // skip identifier (1 byte)
        	      bytes.getByte();
...
function _fromAsn1WithoutContext(msg, obj, validator) {
	  var capture = {};
	  var errors = [];
	  if(!asn1.validate(obj, validator, capture, errors)) {
	    var error = new Error('Cannot read PKCS#7 message. ' +
...
define(['./aes','./asn1','./des','./oids','./pem','./pkcs7asn1','./random','./util','./x509'], function() {
