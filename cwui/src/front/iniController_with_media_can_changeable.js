/* 
 * 매체 리스트 클릭시 현재 behavior 가 save 일 경우는 매체 타켓만 조정하며, 해당 매체의 인증서 리스트는 그리지 않음 
 * 즉 자유롭게 복사나 가져오기시에 나중에 저장할 매체를 선택할 수 있는 버젼
 * */ 
define([
	'../main/constants',
	'../front/iniAction',
	...
	'../main/system'
	], function(constants, iniAction, iniTemplate, iniCertManager,  inputLayout, messageBundles, messageDialog, preProcessor,postProcessor, msgFactory, defaultConf, keyStrokeSecurityFactory, system) {
	"use strict";
	var _handleInfo, 
		_pin, 
		_targetPin, 
	 ...
	 */  
	function setButtonLayout(buttons){
		_elems.$DLG_CONTAINER.find("button").hide();  
		if (buttons['length']){
	...
	*******************************************************************************************/
	...
	function processCertManager(){
		...
		var behavior = _handleInfo.serviceInfo.getBehavior();
		var taskNm  = _handleInfo.requestInfo.getParameter("taskNm");
		var taskNm = behavior ? behavior : taskNm ;
		/* 해당 태스크 작업이 완료 되면, 관련 파라미터 초기화*/
	...
	function onFormLoad(){ 
		...
		case constants.WebForm.ACTION_SIGN: 
			...
			if(_handleInfo.serviceInfo.getDeviceId() === constants.Certs.STORAGE_USIM){
	        	...
	        	_handleInfo.serviceInfo.setExceptionCallback(exceptionHandler);
	        	iniAction.signature(_handleInfo);
	...
	function selectStorage(pId, sId) {
		...
		if (!selectedStorage  || selectedStorage  == 'N'){
			...
			case constants.WebForm.ACTION_MANAGE: 
				setButtonLayout(['cancel']);
				if (constants.WebForm.ACTION_CERT_SAVE === behavior){
					$('#INI_certificate_area').hide();
					$("#INI_change_area").show();
					_cancelHandler = previousCancelHandler;
			...
			case constants.WebForm.ACTION_CERT_CMP: 
				...
				if ( _handleInfo.serviceInfo.getDeviceId() === constants.Certs.STORAGE_SECURITY_DEVICE ||
						_handleInfo.serviceInfo.getDeviceId() === constants.Certs.STORAGE_SCARD ||  
						_handleInfo.serviceInfo.getDeviceId() == pId ) {
					inputKeys.push('SECURE')
				}
				inputLayout.showKeyInputLayout(inputKeys);
				break;
			default: 
				if (action === constants.WebForm.ACTION_LOGIN) {
					if (behavior ===constants.WebForm.ACTION_CERT_SAVE ) {
						$('#INI_certificate_area').hide();
						$("#INI_change_area").show();
						_cancelHandler = previousCancelHandler;
						if (constants.Certs.STORAGE_SCARD == pId || 
	...
	function onManagerClicked(manager,e){
		...
		case "CERT_COPY":
			if (!hasSelectedCert()) return;
			view ="destination_storage_select";
			viewType = "MODAL";
			_handleInfo.serviceInfo.setBehavior(constants.WebForm.ACTION_CERT_COPY);
			openCallback = function (handleInfo){iniCertManager.drawTargetStorageList()};
			break;
		
		case "CERT_REMOVE":
			if (!hasSelectedCert()) return;
			_handleInfo.optionInfo.setParameter("CERT_VIEW_TYPE", constants.Certs.CERTIFICATE_NORMAL_ATTR);
			_handleInfo.serviceInfo.setBehavior(constants.WebForm.ACTION_CERT_REMOVE);
		...
		case "CERT_PWD_CHANGE": 
			if (!hasSelectedCert()) return;
			_handleInfo.serviceInfo.setBehavior(constants.WebForm.ACTION_CHANGE_PWD);
		...
		case "CERT_EXPORT":
			...
			view = 'cert_export'; 
			_handleInfo.optionInfo.setParameter("CERT_VIEW_TYPE", constants.Certs.CERTIFICATE_NORMAL_ATTR);
			_handleInfo.serviceInfo.setBehavior(constants.WebForm.ACTION_CERT_EXPORT_FILE);
			inputLayout.showKeyInputLayout(['NONCE']);
			openCallback = function(handleInfo){
				iniCertManager.getCertAttributeInfo(handleInfo);
				setButtonLayout(['ok','cancel']);
	...
	function certManualSearch(){
		...
		var openCallback = function(handleInfo){
			handleInfo.serviceInfo.setParameter("certSelectedCallback", certSelectedCallback);
	...
	function certSelectedCallback(handleInfo){
